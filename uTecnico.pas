...
type
  ...
  TForm4 = class(TForm)
    ...
    Panel5: TPanel;
    Label47: TLabel;
    edt_us_pesquisa_ra: TEdit;
    Label48: TLabel;
    edt_us_pesquisa_nome: TEdit;
    Label49: TLabel;
    edt_us_pesquisa_email: TEdit;
    btn_Us_AtualizarTecnico: TSpeedButton;
    ...
    procedure btn_Us_AtualizarTecnicoClick(Sender: TObject);
    procedure btn_desatribuirClick(Sender: TObject);
  private
    ...
    function BuscarUsuario(RA: String): TUsuario;
    procedure PreencherCamposUsuarioPesquisa;
    procedure AtualizarUsuarioTecnico(Tecnico: Integer; RA: string);
    procedure DesatribuirChamado(id_Chamado: Integer);
...
procedure TForm4.btn_pesquisarClick(Sender: TObject);
begin

  // Verificar se o RA já existe no banco de dados
  if VerificarRACadastrado(edt_pesquisa.Text) then
  begin
    Form4.TabControl1.TabIndex := 3;
    Form4.TabControl4.TabIndex := 1;
    PreencherCamposUsuarioPesquisa;
  end
  else
  begin
    ShowMessage('O RA informado não está cadastrado. Por favor, insira um RA valido.');
    edt_pesquisa.Text := '';
    Exit;
  end;
...
begin
    ...
    FDQChamado.ParamByName('Situacao').AsString := 'Atribuido';
...
begin
  ...
  begin
    ...
    vUsuario.Tecnico := FDQUsuario.FieldByName('Tecnico').AsInteger
  end;
  Result := vUsuario;
end;
...
function TForm4.BuscarUsuario(RA: String): TUsuario;
var
  vUsuario: TUsuario;
begin
  FDQUsuario.Close;
  FDQUsuario.SQL.Clear;
  FDQUsuario.SQL.Add('SELECT * FROM Usuario WHERE RA = :RA');
  FDQUsuario.ParamByName('RA').AsString := RA;
  FDQUsuario.Open;
  if not FDQUsuario.IsEmpty then
  begin
    vUsuario.ID := FDQUsuario.FieldByName('ID').AsInteger;
    vUsuario.Nome := FDQUsuario.FieldByName('Nome').AsString;
    vUsuario.RA := FDQUsuario.FieldByName('RA').AsString;
    vUsuario.Email := FDQUsuario.FieldByName('Email').AsString;
    vUsuario.Tecnico := FDQUsuario.FieldByName('Tecnico').AsInteger;
...
procedure TForm4.PreencherCamposUsuarioPesquisa;
var
  vUsuario: TUsuario;
begin
  vUsuario := BuscarUsuario(edt_pesquisa.Text);
  edt_us_pesquisa_nome.Text := vUsuario.Nome;
  edt_us_pesquisa_ra.Text := vUsuario.RA;
  edt_us_pesquisa_email.Text := vUsuario.Email;

  if vUsuario.Tecnico = 1 then
  begin
    cbTecnico.IsChecked := True;
  end
  else
  begin
    cbTecnico.IsChecked := False;
  end;
end;
...
///////////////////////////////////////////////////////BOTÃO PARA DESATRIBUIR CHAMADO///////////////////////////

procedure TForm4.btn_desatribuirClick(Sender: TObject);
begin
//retirar da lista do tecnico
  DesatribuirChamado(StrToInt(lbl_tec_enc_nChamado.Text));
// voltar para lista e atualizar
  Form4.TabControl2.TabIndex := 0;
  buscarChamadosTecnico;
...
procedure TForm4.DesatribuirChamado(id_Chamado: Integer);
begin
  FDQChamado.Close;
  FDQChamado.SQL.Clear;
  FDQChamado.SQL.Add('UPDATE Chamado SET');
  FDQChamado.SQL.Add('Tecnico = :Tecnico,');
  FDQChamado.SQL.Add('Situacao = :Situacao,');
  FDQChamado.SQL.Add('IdTecnico = NULL');
  FDQChamado.SQL.Add('WHERE ID_Chamado = :ID');

  FDQChamado.ParamByName('Tecnico').AsString := 'Nenhum';
  FDQChamado.ParamByName('Situacao').AsString := 'Aberto';
  FDQChamado.ParamByName('ID').AsInteger := id_Chamado;

  try
    FDQChamado.ExecSQL;
    ShowMessage('Chamado desatribuído com sucesso.');
  except
    on E: Exception do
    begin
      ShowMessage('Erro ao desatribuir chamado: ' + E.Message);
      Exit;
    end;
  end;
end;
...
procedure TForm4.btn_Us_AtualizarTecnicoClick(Sender: TObject);
var tecnico : integer;
begin

  if cbTecnico.IsChecked then
  begin
    tecnico := 1;
  end
  else
  ...
  AtualizarUsuarioTecnico(tecnico, edt_pesquisa.Text);
  PreencherCamposUsuarioPesquisa;
end;
...
procedure TForm4.AtualizarUsuarioTecnico(Tecnico: Integer; RA: string);
begin
    FDQUsuario.Close;
    FDQUsuario.SQL.Clear;
    FDQUsuario.SQL.Add('UPDATE Usuario SET');
    FDQUsuario.SQL.Add('Tecnico = :Tecnico');
    FDQUsuario.SQL.Add('WHERE RA = :RA');
    FDQUsuario.ParamByName('Tecnico').AsInteger := Tecnico;
    FDQUsuario.ParamByName('RA').AsString := RA;
...
begin
  ...
  FDQChamado.SQL.Add('AND Situacao <> ''Encerrado''');
...
function TForm4.VerificarRACadastrado(RA: string): Boolean;
begin
  FDQUsuario.Close;
  FDQUsuario.SQL.Clear;
